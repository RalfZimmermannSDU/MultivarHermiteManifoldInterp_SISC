function [Wlocs, samplocs, d1samplocs, d2samplocs, pstar, logs_array, d1_logs_array, d2_logs_array] = create_sample_data_Sphere_tang(test_fun, Wspace, N, dim)
%
...
% dimension of Sphere embedding space
d = 3;

Wlocs = zeros(dim,N);
samplocs   = zeros(d,N);
d1samplocs = zeros(d,N);
d2samplocs = zeros(d,N);
...
tau = 1.0e-8;
[pstar, count, fail] =  findCenter_Sphere(samplocs, weights, q0, tau)
...
d1coeffs = zeros(N,N);
d2coeffs = zeros(N,N);
logs_array = zeros(d,N);   % storage space for Riemann logs
    
%map all data to T_(pstar)S
for i = 1:N     
    Logi = log_sphere(pstar,samplocs(:,i));
...
d1_logs_array = zeros(d,N);
d2_logs_array = zeros(d,N);

for i=1:N
    % data for central difference approximation
    h = 1.0e-5;
    Tplus = exp_sphere(samplocs(:,i), 1, h*d1samplocs(:,i));
    fplus = log_sphere(pstar, Tplus);
    Tminus= exp_sphere(samplocs(:,i), 1, (-h)*d1samplocs(:,i));
    fminus= log_sphere(pstar, Tminus);
    % central difference approximation   
    d1_logs_array(:,i) = (1.0/(2*h))*(fplus - fminus);

    % for second dim
    Tplus = exp_sphere(samplocs(:,i), 1, h*d2samplocs(:,i));
    fplus = log_sphere(pstar, Tplus);
    Tminus= exp_sphere(samplocs(:,i), 1, (-h)*d2samplocs(:,i));
    fminus= log_sphere(pstar, Tminus);
    % central difference approximation   
